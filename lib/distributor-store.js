...
function cleanPhone(value) {
  ...
  if (/^[2-8]\d{7}$/.test(digits)) return `506${digits}`;
  if (/^506[2-8]\d{7}$/.test(digits)) return digits;
  return "";
}
...
async function updateOwnProfile(codeValue, input) {
  ...
  if (input.password) {
    if (String(input.password).length < 4) throw new Error("La contraseña debe tener al menos 4 caracteres");
